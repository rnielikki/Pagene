    ...
    partial class TagManager
    {
        private void Deserialize()
        {
            ...
            foreach (var file in files)
            {
                ...
                var (tag, posts) = JsonSerializer.Deserialize<(string tag, List<BlogEntry> posts)>(stream);
        ...
        internal async System.Threading.Tasks.Task Serialize()
        {
            ...
            {
                var item = (tagPair.Key, tagPair.Value.Values);
                using var file = _fileSystem.File.Open($"{_dirName}/{item.Key.ToLower()}.json", FileMode.Create);
